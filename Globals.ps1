#------------------------------------------------------------------------------
# Set Script Global Variables and Functions here
#------------------------------------------------------------------------------
...
# Function - Add Text to RichTextBox. Add separator to output text box.
function Add-RichTextBox1
{
	[CmdletBinding()]
	param ($text)
	#$richtextbox1_output.Text += "`tCOMPUTERNAME: $ComputerName`n"
	$richtextbox1_output.Text += "$text"
	$richtextbox1_output.Text += "`n# # # # # # # # # #`n"
}
...
{
	$global:datesortable = Get-Date -Format "ddMMyyyy-HH':'mm':'ss"
...
function Add-Logs
{
	...
	param ($text)
	Get-datesortable
	$richtextbox1.Text += "[$global:datesortable] - $text`r"
	Set-Alias alogs Add-Logs -Description "Add content to the RichTextBox1"
	Set-Alias Add-Log Add-Logs -Description "Add content to the RichTextBox1"
}#endregion Add Logs



#:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
#region Clear-RichTextBox1
# Function - Clear the RichTextBox1
function Clear-RichTextBox1 { $richtextbox1.Text = "" }
...
function Clear-Logs { $richtextbox1_logs.Text = "" }
...
function Start-Proc
{
	param (
		[string]
		$exe = $(Throw "An executable must be specified"),
		[string]
		$arguments,
		[switch]
		$hidden,
	...
	Process
	{
		...
		if (Test-Host $ComputerName -TCPPort 135)
		{
			...
			try
			{
				...
				$OSVersionInfo = Get-WmiObject Win32_OperatingSystem -ComputerName $ComputerName -Property Version -ErrorAction STOP
...
function Get-Processor
{
	...
	Process
	{
		...
		if (Test-Host -ComputerName $ComputerName -TCPPort 135)
		{
			try
			{
				$CPUS = Get-WmiObject Win32_Processor -ComputerName $ComputerName -ErrorAction STOP
				foreach ($CPU in $CPUs)
				{
					$myobj = @{
						ComputerName = $ComputerName
...
function Test-Host
{
	...
	Begin
	{
		...
		function PingServer
		{
			...
			try
			{
				$pingresult = Get-WmiObject win32_pingstatus -Filter "address='$MyHost'"
...
function Get-MemoryConfiguration
{
	...
	Process
	{
		...
		if (Test-Host $ComputerName -TCPPort 135)
		{
			...
			try
			{
				$MemorySlots = Get-WmiObject Win32_PhysicalMemory -ComputerName $ComputerName -ErrorAction STOP
		...
		switch -regex ($NetStat)
		{
			$RegEx
			{
				...
				$myobj = @{
					...
					ProcessName = Get-Process -id $matches.PID -ErrorAction 0 | ForEach-Object{ $_.name }
				}
...
function Get-MotherBoard
{
	...
	Process
	{
		...
		if (Test-Host -ComputerName $ComputerName -TCPPort 135)
		{
			try
			{
				$MBInfo = Get-WmiObject Win32_BaseBoard -ComputerName $ComputerName -ErrorAction STOP
				$myobj = @{
					ComputerName = $ComputerName
					Name		 = $MBInfo.Product
	...
	process
	{
		...
		if (Test-Host $ComputerName -TCPPort 135)
		{
			try
			{
				if ($Last)
				{
					$date = Get-WmiObject Win32_OperatingSystem -ComputerName $ComputerName -ErrorAction STOP | ForEach-Object{ $_.LastBootUpTime }
				...
				else
				{
					...
					Get-WmiObject -Query $Query -ErrorAction 0 -ComputerName $ComputerName | ForEach-Object {
						$myobj = @{ }
...
function Get-NetworkSummary ([String]$IP, [String]$Mask)
{
	...
	Add-Member NoteProperty "Network" -InputObject $NetInfo -Value $NetworkAddress
	Add-Member NoteProperty "Broadcast" -InputObject $NetInfo -Value $BroadcastAddress
	Add-Member NoteProperty "Range" -InputObject $NetInfo `
			   -Value "$RangeStart - $RangeEnd"
	Add-Member NoteProperty "Mask" -InputObject $NetInfo -Value $Mask
	Add-Member NoteProperty "MaskLength" -InputObject $NetInfo -Value $MaskLength
	Add-Member NoteProperty "Hosts" -InputObject $NetInfo `
			   -Value $($Broadcast - $Network - 1)
	Add-Member NoteProperty "Class" -InputObject $NetInfo -Value $Class
	Add-Member NoteProperty "IsPrivate" -InputObject $NetInfo -Value $Private
...
{
	Param ([string]
		$message = $(Throw "You must enter a prompt message"),
		[string]
		$title = "Input",
		[string]
		$default
